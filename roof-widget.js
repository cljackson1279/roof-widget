...
/* Instant Roofing Estimate — compact alignment, size presets (full/compact/ultra), modal, brand color detection */
...
(function () {
  ...
  const cfg = {
    ...
    width: me?.dataset?.width || "560px",                      // e.g. 360px, 520px, 100%
    primaryOverride: me?.dataset?.primary || "",               // override brand color
    modalButton: (me?.dataset?.modalButton || "").toLowerCase()// "auto" for floating CTA
  };

  // --- Brand color detection (fallback to override/default) ---
  ...
  // --- Styles (tight spacing + consistent alignment) ---
  const css = `
  ...
  /* ultra (single column, extra tight) */
  ...
  // --- Build form (labels stacked over inputs, two columns) ---
  function buildForm(){
  ...
  async function onSubmit(e, root){
    ...
    function showError(m){ if(err){err.textContent=m; err.style.display="";} }
    ...
    const phoneE164 = toE164US(payload.phone); if(!phoneE164) return showError("Please enter a valid 10-digit US phone number.");
    ...
    try{
      ...
      console.error(e); showError("Sorry, we couldn't calculate your estimate. Please try again.");
    }finally{
  ...
  function buildModalAndMount(formWrap){
    ...
    if(externalBtn) externalBtn.addEventListener("click",open);
      ...
      const fab=document.createElement("button"); fab.className="rw-fab"; fab.textContent="Get Instant Estimate";
  ...
  if(cfg.variant==="modal"){ buildModalAndMount(wrap); }
  else{ const container=me.parentElement||document.body; container.insertBefore(wrap,me); }
})();
